...
## Unity 연동 가이드 (UPM 패키지 배포)
이 플러그인은 Unity Package Manager(UPM)를 통해 배포할 수 있도록 설계되었습니다. GitHub를 통해 배포되는 Unity 패키지의 권장 구조는 다음과 같습니다.

```text
packages/work.ctrl3d.camerax/
├── package.json
├── README.md
├── CHANGELOG.md
├── LICENSE.md
├── Runtime/
│   ├── work.ctrl3d.camerax.asmdef
│   ├── AndroidNativeCameraBridge.cs
│   └── NativeCameraController.cs
├── Editor/
│   ├── work.ctrl3d.camerax.Editor.asmdef
│   └── NativeCameraPluginDependencies.xml
└── Plugins/Android/
    ├── NativeCameraPlugin-release.aar
    └── libs/
        ├── arm64-v8a/liboes_renderer.so
        └── armeabi-v7a/liboes_renderer.so
```
